   ...
   await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
     .then(row => {
       if(typeof(row[0]) !== "undefined"){
        session.send(h('quote',{id:session.messageId}) + `【${poolName}】池子名已存在,创建失败`) 
       }else{
...
ctx.command('poolmanager.delete <poolName:string>','删除卡池').alias('删除卡池')
 ...
 .action(async ({session},poolName) => {
   await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
     .then(row => {
       if(typeof(row[0]) !== "undefined"){ return row[0] }else{ session.send(h('quote',{id:session.messageId}) +`输入卡池名称不存在`) }
     })
     .then(result => {
       if (result.owner !== session.userId){
        session.send(h('quote',{id:session.messageId}) +`非卡池创始人无权删除`)
       }else{
...
.action(async({session},poolName,rarity) => {
  await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
    ...
    .then(result => {
      if (result.owner === session.userId){ return result }else{ session.send(h('quote',{id:session.messageId}) +`非卡池创始人不可设置`) }
    })
    .then(result => {
      let probability = {},
      ...
      if(JSON.stringify(goodsList) !== "{}"){
        for(let item in probability){
          if (!(item in goodsList)){
            session.send(h('quote',{id:session.messageId})+`【${item}】不存在于goodsList字段,设置失败`)
            return 
          }
        }
      }
...
.action(async({session},poolName,rarity,goods) => {
  await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
    ...
    .then(result => {
      if (rarity in result.probability ){ return result }else{ session.send(h('quote',{id:session.messageId}) +`【${poolName}】中不存在稀有度【${rarity}】`) }
    })
    .then(result => {
      if (result.owner === session.userId){ return result }else{ session.send(h('quote',{id:session.messageId}) +`非卡池创始人不可更改`) }
    })
    .then(result => {
      let goodsList = result.goodsList,
          odds = goods.split(' ');
      if(typeof(goodsList[rarity]) === 'undefined'){ goodsList[rarity] = [] }
      ...
      session.send(h('quote',{id:session.messageId})+`成功为【${poolName}】添加物品`)
    })
})
...
.action(async({session},poolName,rarity,goods) => {
  await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
    ...
    .then(result => {
      ...
      ctx.database.set('pool_simulator_table',{poolName:[poolName]},{goodsList:goodsList})
      session.send(h('quote',{id:session.messageId})+`成功为【${poolName}】删除物品`)
    })
})
...
ctx.command('poolmanager.show','展示所有卡池').alias('展示所有卡池')
 ...
 .action(async({session},poolName) => {
   await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
     .then(row => {
       if(typeof(row[0]) === "undefined"){
        session.send(h('quote',{id:session.messageId})+`输入卡池名称不存在`) 
       }else{
        let messageList = [h('message',`【${poolName}】卡池配置如下：`)],
            probability = row[0].probability,
            goodsList = row[0].goodsList;
        for( let item in probability ){ messageList.push( h( 'message',`${item}(${probability[item]}%):${goodsList[item]}` ) ) }
        session.send(h('message',{forward:true},messageList))
       }
     })
 })
 ...
 .action(async({session},poolName,number) => {
   await ctx.database.get('pool_simulator_table',{poolName:[poolName]})
     ...
     .then(result =>{
      ...
      if(JSON.stringify(probability) === "{}"){session.send(h('quote',{id:session.messageId})+`卡池概率未配置,抽卡失败`);return;}
      if(JSON.stringify(goodsList) === "{}"){session.send(h('quote',{id:session.messageId})+`卡池物品未配置,抽卡失败`);return;}
      for(let item in probability){if(goodsList[item] === undefined){session.send(h('quote',{id:session.messageId})+`卡池物品未配置完全,抽卡失败`);return;}}
      if(number > 100) { session.send(h('quote',{id:session.messageId})+`抽卡次数不能大于100`);return;}

      for(let i=1;i<=number;i++){
        let outcome = Draw(goodsList,probability),
            message = h('message',`第${i}次抽卡结果:【${outcome[0]} ${outcome[1]}】`);
